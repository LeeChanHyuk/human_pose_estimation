...
def main(color=(224, 255, 255), rgb_video_path = 'save.avi', depth_video_path = 'save.avi', save_path = 'data', image_path = None):
    base_path = os.getcwd()
    zmq_enable = True
    write_count = 0
    context = zmq.Context()
    ...
    with mp_pose.Pose(
        ...
        with mp_face_mesh.FaceMesh(
            ...
            while True:
                if image_path is not None and frame_count>len(image_path):
                    break
                ...
                if use_realsense:
                    ...
                    frame = cv2.imread(image_path[frame_count])
                ...
                if visualization and all_estimation_is_successed:
                    ...
                    if gaze_estimation:
                        for i, ep in enumerate([left_eye, right_eye]):
                            ...
                            gaze = gazes[i]
                            length = 120.0
                            draw_gaze(frame, ep.landmarks[-2], gaze, length=length, thickness=10)
                ...
                cv2.imwrite(image_path[frame_count]+'_1.jpg', frame)
                frame_count += 1
...
def main_function():
    if annotation:
        ...
        if image_inference:
            image_list = []
            for file in os.listdir('C:/Users/user/Desktop/action2'):
                image_list.append('C:/Users/user/Desktop/action2' + '/' + file)
            main(image_path = image_list)
        elif not use_video:
            main()
        elif use_realsense:
            main()
