...
for file in tqdm(os.listdir(os.path.join(train_file_path, 'train_video'))):
    cap = cv2.VideoCapture(os.path.join(train_file_path, 'train_video', file))
    os.mkdir(os.path.join(train_file_path, 'train_frames', file[0:len(file)-4]))
    ...
    while True:
        ...
        if frame is None:
            break
        cv2.imwrite(os.path.join(train_file_path, 'train_frames', file[0:len(file)-4], str(frame_name)+'.jpg'), frame)
        frame_name += 1
